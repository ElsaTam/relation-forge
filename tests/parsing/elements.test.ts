...
describe('Verify that characters are parsed correctly', () => {
	...
	test('Should create a character with the proper description and status', () => {
		...
		expect([
			...
			character.status
		]).toStrictEqual(["Testing character", "dead"]);
	});
...
describe('Verify that relations are parsed correctly', () => {
	...
	test('Should use the default values for numeric attributes that are non-existing', () => {
		...
		DEFAULT_SETTINGS.rangeProperties.frequency.default = 2;
		DEFAULT_SETTINGS.rangeProperties.affinity.default = 4;
		DEFAULT_SETTINGS.rangeProperties.trust.default = -3;
		DEFAULT_SETTINGS.rangeProperties.impact.default = -6;
		const relations = Relation.fromPage(page, DEFAULT_SETTINGS);
		expect(relations.length).toBe(1);
		...
		expect([
			...
			page.character_1.path,
			"character",
			DEFAULT_SETTINGS.rangeProperties.influence.default,
			DEFAULT_SETTINGS.rangeProperties.frequency.default,
			DEFAULT_SETTINGS.rangeProperties.affinity.default,
			DEFAULT_SETTINGS.rangeProperties.trust.default,
			DEFAULT_SETTINGS.rangeProperties.impact.default,
		]);
	});
});
