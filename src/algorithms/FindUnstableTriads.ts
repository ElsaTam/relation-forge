            ...
            for (let j = i + 1; j < characters.length; j++) {
                for (let k = j + 1; k < characters.length; k++) {
                    ...
                    unstableTriads.push({
                        ...
                        relationships: [
                            { source: char1.id, target: char2.id, influence: newRange('influence', rel12?.influence), label: 'relation', type: type12 },
                            { source: char2.id, target: char3.id, influence: newRange('influence', rel23?.influence), label: 'relation', type: type23 },
                            { source: char3.id, target: char1.id, influence: newRange('influence', rel31?.influence), label: 'relation', type: type31 }
                        ],
                        isBalanced,
                        score: balanceScore,
                        tension
